                    ...
                    selectedAppFolderID = nil; showingFavoritesOnly = false
                           ...
                           .contextMenu {
                               ...
                               Button(action: {
                                   ...
                                   if !targets.isEmpty {
                                       selectedFileIDs = Set(targets.map { $0.id })
                                       exportSelectedFiles()
                                   }
                    ...
                    ForEach(filtered) { file in
                        ZStack(alignment: .bottomTrailing) {
                            FileThumbnailView(file: file) {
                                ...
                                } else if let idx = filtered.firstIndex(where: { $0.id == file.id }) {
                                    galleryIndex = idx
                                    showingGallery = true
                                }
                            }
                            .contextMenu {
                                if !isSelectionMode {
                                    ...
                                    Button(role: .destructive, action: {
                                        selectedFileIDs = [file.id]
                                        showingMultiDeleteConfirm = true
                                    }) {
                                        Label("削除", systemImage: "trash")
                                    }
                                }
                            ...
                            if !isSelectionMode {
                                Button(action: {
                                    withAnimation(.spring(response: 0.3, dampingFraction: 0.5)) {
                                        if favoriteFileIDs.contains(file.id) {
                                            favoriteFileIDs.remove(file.id)
                                        } else {
                                            favoriteFileIDs.insert(file.id)
                                        }
                                        saveFolders()
                                    }
                                }) {
            ...
            ToolbarItem(placement: .navigationBarTrailing) {
                if isSelectionMode {
                    ...
                    Menu {
                        ...
                        Button(action: { showingNotificationSetup = true }) {
                            Label("自爆の通知", systemImage: "bell.badge")
                        }
